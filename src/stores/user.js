...
export const useUserStore = defineStore('user', {
  ...
  actions: {
    ...
    async fetchAllUserData() {
      ...
      try {
        const [tags, playlists, likes, profile] = await Promise.all([
          sb.getUserTagsWithTracks(this.user.id),
          sb.getUserPlaylistsWithTracks(this.user.id),
          sb.getUserLikes(this.user.id),
          sb.getProfile(this.user)
        ])
        this.tags = tags || []
    ...
    async toggleTagged(tagId, track) {
      ...
      if (!tag) return
      const isTrackInTag = tag.tracks.some(tr => tr?.id === track.id.toString())
      try {
        isTrackInTag ? await sb.untagTrack(tagId, track.id) : await sb.tagTrack(track, tagId, this.user.id)
